...
public class When_generating_a_Builder_with_readonly_constructor_parameters : Given_a_BuilderGenerator
{
    [Fact]
    public Task Test()
    {
        ...
        var inputCompilation = CreateCompilation(GetResourceAsString(assembly, "ExampleWithReadOnlyConstructor.cs"))
            .AddSyntaxTrees(CSharpSyntaxTree.ParseText(GetResourceAsString(assembly, "PersonWithReadOnlyConstructorBuilder.cs")));
        ...
        outputCompilation.SyntaxTrees.Count().ShouldBe(3); // The entity, the builder stub, and the output
